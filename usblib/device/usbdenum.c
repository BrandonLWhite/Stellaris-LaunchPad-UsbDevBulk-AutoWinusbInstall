...
USBDGetDescriptor(void *pvInstance, tUSBRequest *pUSBRequest)
{
    ...
    switch(pUSBRequest->wValue >> 8)
    {
        ...
        case USB_DTYPE_STRING:
        {
            ...
            //
            // If the mapping function returned -1 then call the handler for string requests that are not
            // handled from the string descriptor table, if one has been furnished.
            // Otherwise stall the request to indicate that the request was not valid.
            //
            if(lIndex == -1)
            {
            	if(psDevice->sCallbacks.pfnGetStringDescriptor)
            	{
					psDevice->sCallbacks.pfnGetStringDescriptor(psUSBControl->pvInstance,
														        pUSBRequest);
					return;
            	}
            	else
